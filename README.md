...
| **Checklist attributes** | A record's TAG/VALUE attribute list — a sample's collection date, host, isolation source, whatever its checklist defines — arrives as `attr:`-prefixed columns, hidden until ticked on in *Columns*. Neither listing API has these: the Reports API returns five columns per record, and the Portal API indexes a fixed set (102 fields for a sample), so a tag outside that set is in the record XML and nowhere else. Fetched with the rows in read mode as well as write mode, in batches of 100, and **read-only** — ENA would accept a MODIFY that changed one, but `ena_submission_toolkit` addresses an editable field by a fixed path per entity, and an attribute is addressed by its tag's text inside a list that differs per record. |
